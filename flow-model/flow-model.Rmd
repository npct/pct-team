...
Yet for infrastructure and other 'space for cycling' measures to be effective,
...
the rate of cycling use at area, desire line, route and route network levels,
currently and under a range of 'cycling futures', which represent different levels of ambition.
Scenarios including 'Go Dutch' and 'Ebikes' (which explored the impact of electric cycles on cycling uptake) were developed and visualised.
The PCT's use of open source software enables others to create alternative scenarios, for example to represent the route network capacity implications of acheiving local cycling targets and could facilitate its deployment in new cities and countries where origin-destination data is available.
We conclude with a discussion of how new interactive and online tools such as the PCT could help make transport planning a more open and citizen led process.
...
In this context there is growing interest, and in some cases investment,
...
Providing high-quality infrastructure can play a key role in cycling uptake [@parkin_cycling_2012].
...
There is a wide range of interventions related to infrastructure which can be grouped the banner 'space for cycling' [@parkin_planning_2015].
...
Design options include the width [@wegman_urban_1979;@pikora_developing_2002], quality [@heath_effectiveness_2006], directness  [@crow_design_2007] and geographic location of the paths.
Much evidence and guidance has been produced about the design of cycle paths [e.g. @transport_for_london_london_2015;@welsh_government_guidance_2014].
Yet little work has explicitly tackled the question of where this high quality infrastructure should be built
[@aultman-hall_analysis_1997;@minikel_cyclist_2012].
The PCT focusses explicitly on the question of *where* to build rather than *what* to build (although it does provide evidence on future capacity for different route segments, which is related to width).
...
been shortlisted. Unlike the prioritization index of Larsen et al. (2013), which is primarily aimed at informing a strategic cycling network, the results of the PAT are designed to guide smaller, site specific interventions such as 'contraflow' paths and cyclist priority traffic signals.

At the opposite end of the scale spectrum, @parkin_estimation_2008 created a regression model to estimate the proportion of commuter trips cycled across wards in England and Wales.
Ethnic minorities, low income, road defects, rainfall and hills were all associated with low levels of cycling, leading to the conclusion that policy makers must engage with a mixture of social and physical barriers to effectively promote cycling, with the implication that some areas are more likely to respond to investment than others.
...
Ambitious local and national cycling targets are often based on mode
share.^[The
...
- Route-allocation of the desire lines by an routing algorithm specifically developed for cycling (thanks to CycleStreets.net).
...
- Its level of accessibility. The tool is publicly available on-line and developers are encourage to modify the PCT (e.g. to create alternative scenarios) by provision of the source code underlying the PCT under the open source AGP License.

As with any tool, its utility depends on people knowing how to use it.
...
Further enhancements to the Manual are planned following user feedback.
...
and hillines was used to create plausible scenarios of cycling futures.
...
and 'hilliness decay curves' (illustrated in Figure 2) allowed estimates of the model split
of cycling to generated for every route-allocated OD pair.
...
The PCT focusses explicitly on the latter scenario as it is (currently)
...
Tables 1 and 2 illustrate the two tables that comprise the OD data. Figure 2
...
Information about the cycling potential along specific routes between OD pairs provides a useful evidence base for informing where new cycle paths should be built: planners can select the 'top n' routes with the highest estimated cycing demand.
However, even more useful is information about the *aggregate cycling potential* on specific segments on the road network.
By adding together the overlapping flows from multiple OD pairs a 'route network' of cycling potential can be created. This can result in much higher estimates of cycling potential than the route-allocated OD data alone. This is illustrated in Figure 5, which shows the rate of cycling allong Otley Road rise from 73 to 296 based on a single OD pair but a much larger absolute rise of 301 to 1133 when all route-allocated OD pairs are considered.
For the route allocation process the R function `overline` was written [@lovelace_stplanr:_2016].^[Thanks to Barry Rowlingson from the University of Lancaster for assistance developing this function.]
...
These were informed by the the policy context of a government target to double the number of cycle trips [@department_for_transport_cycling_2014] and evidence from overseas of the proportion of trips of certain distances that *could* be made by cycling, especially in relation gender equality in cycling rates and uptake of electric cycles (Ebikes).
...
The concept of 'distance decay' [@iacono_access_2008]
...
currently, and under scenarios of the future.
Note that Oxfordshire, which has a high rate of cycling,
has a tall and steep distance decay curve
whereas West Yorkshire has a low and comparatively shallow curve,
for the current rate of cycling. Note also differences in the
trip-distance distribution overall between the two regions,
suggesting that a higher proportion of trips overall are cyclable
in Oxfordshire than in West Yorkshire.

There is noticeable distance decay
for all modes of transport, especially in Oxfordshire which has a
noticeable peak in trips between 2 and 3 km long, well within cycling distance.
The visual presentation of trip distance distributions in the PCT (in the Model Output tab)
can be used to support the argument that there is high cycling potential in both Oxfordshire and West Yorkshire:
because a large proportion of trips are within cyclable distance,
there is a strong case that cycling investment can be justified.
...
Note that although West Yorkshire and Oxfordshire have very
...
(*godutch*) scenarios is illustrated in Figure 7 for Leeds.
The top 6 OD pairs (6 was used to allow focus on the city centre)
in Leeds in *govtarget* are dominated by the current rate of cycling.
...
knitr::include_graphics("man-cdp-dutch.png")
```
...
Users can view a summary of the model via a 'Model Output' tab.
...
with maps showing the shifting distribution of cyling
...
- Aditional scenarios to illustrate a wider range of 'cycling futures', including medium-term and local targets such as 'Go York' (where 12% of commuters cycle to work).
...
- Use of alternative input data data, such as the location of new developments and road traffic speed and volumes. This work could build on recently developed GIS for transport planning [@farrell_evaluating_2015].
...
In conclusion, although the PCT aimed to inform the narrow question of where to construct
