...
Instead of running the model for the entirety of England, the script to
load the national data into memory, and generate the outputs for visualisation,
ran on a region-by-region basis (see [github.com/npct/pct-load/](https://github.com/npct/pct-load/blob/master/build_region.R)).
The regional focus was selected because transport decisions
tend to be made at the local level [@Gaffron2003] and to reduce the
computational requirements of each regional 'build' for scalability.
Using only one regional geographic level also has limitations,
...
to enable 'builds' for various geographical levels. London, for example,
was 'built' at 2 different levels, one covering the entirety of the city
and another which divided the city into 4 quadrants
