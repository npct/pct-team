...
dft = read.csv("../data-sources/160516_cycle-tools.csv")
# knitr::kable(dft)
library(pander)
...
Note that more confidence can be placed in the relative rather than the absolute size of these numbers: the Route Network layer excludes within-zone commuters, commuters with no fixed workplace, and commuters working in a different PCT region, because these are not represented as 'fast' routes on the travel network.
Route Network values also omit additional potential cycling associated with desire lines outside adjustable selection criteria based on distance and minimum total numbers of all-mode commuters per OD pair.
At the time of writing these were set to 20 km Euclidean distance and 10 commuters respectively.
