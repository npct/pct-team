...
PSS therefore helps tackle the issue that transport models tend to be
...
The PCT primarily relies on origin-destination (OD) data relating the number of trips
taking place between administrative zones. Combined with geographical data on the coordinates of the population-weighted centroid of each zones,
these can be represented as
straight 'desire lines' or (using a route-allocation algorithm) as routes allocated to the transport network.
The route-allocation stage (outlined below) allowed
hilliness and route network distance to be included as additional variables to the OD data.
Unique zone codes were used to link the geographic coordinates of the centroids to the
flows. R package, **stplanr**, was developed to convert the non-geographic
OD data into straight and route-alocated lines that could be visualised in the on-line map [@lovelace_stplanr_2015].
