...
making cycling increasingly central to sustainable transport strategies.
...
Reducing speed limits, creating car free zones and improving the urban realm have all been proposed to make cycling more attractive. However, the intervention that has received the most attention has been the construction of new cycle paths. With devolved transport budgets, local and regional authorities can create and influence the design of strategic cycling networks. Design options include the width [@wegman_urban_1979;@pikora_developing_2002], quality [@Heath2006], directness  [@crow2007] and geographic location of the paths.
...
Much research has explored the impact of infrastructure on
cycling, as outlined in the previous section.
The major challenge from a policy perspective is how to 'operationalise' this body of knowledge, to help planners prioritise where to invest and in what. This research gap was described by
...
There is an emerging literature that seeks to explore the related question of cycling potential. The link is clear: if one can identify which areas/routes/people have the highest potential, this can provide an evidence base about where to invest.
With the notable exceptions of @larsen_build_2013 and @Zhang2014, the work does not generally seek to provide systematic, quantitative or interactive evidence for transport planners. The methods broadly
...
- Individual-based measures are based on individual level survey data, typically a household travel survey. These measures can help target specific groups, e.g. through tailored cycled training schemes, but is not always geographically specific.

- Route-based measures are use origin-destination data which can be used to create 'desire lines' and (using route allocation) estimates of the potential demand at each point on the road network.

None of tools reviewed use route level data as the primary data source.
This work is reviewed below and summarised in Table 1.
...
its cycling potential, the number cycle collisions, and the locations mentioned by
current cyclists of the areas that should be upgraded [@larsen_build_2013].
...
@Payne2014 used the concept of 'filtered permeability' to inform the commercially
...
This, combined with survey data on cyclists' preferences for where to construct new cycle paths and origin-destination data on travel to work, was used to rank key routes in the city in terms of their cycling potential. Although origin-destination data was used in this study, it was not central to the route-allocated estimates of cycling potential as it is in the Propensity to Cycle Tool.
...
In the context of transport planning PSS can be defined as a tool
"that systematically introduces relevant (spatial) information to a
specific process of related planning actions". Thus, the PCT
can be seen as a PSS: it is systematic in the sense that it works in
the same way, using the same input data, for all parts of the study region
(in this case England) and related to a specific planning process ---
where to prioritise investment in cycling and cycle paths.

PSS typically works by presenting evidence about the characteristics and needs
of areas in an interactive map.
Central to the utility of PSS is its ability to visualise the impacts of
alternative scenarios of the future.
The emergence of open source libraries for web mapping [@haklay_web_2008]
has in recent years allowed the PSS methodology to be deployed online and
be potentially available to everyone with an internet connection.
These features of the 'Online What if?' tool for general transport planning
influenced the design of the PCT [@pettit_online_2013].


the limitations of traditional methods (Brommelstroet, 2010).
