...
The PCT uses origin-destination (OD) data to model where cycle traffic has the greatest potential to grow.
Cycling potential was estimated as a function of route distance, hilliness and other factors at the OD and area level, although more sophisticated models could subsequently be visualised throught the PCT.
...
Case study cities were used to illustrate the tool's capabilities under four scenarios, including 'Gender Equality' and 'Go Dutch'.
Critically for policy relevance, the accessible, browser-based interface
...
cycling is the natural choice short trips, as part of a wider shift towards healthy, low carbon transport systems.
...
which in turn could be visualised by the PCT. Although the model is important, it is the
making sense of and visualisation of its output which will most likely lead to improved
decision making.
Therefore, this section should be seen as a case study of one particular approach to modeling particular, context-specific scenarios. Future users of the PCT may choose to use different models of cycling uptake and to examine different scenarios.
...
carbon savings of cycling uptake, building on previous work in the field [@Lovelace2011c;@Singleton2014a].
