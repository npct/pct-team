...
API using the **stplanr** R package.
...
This, and the aggregation of routes to create the 'route network' (see Figure 5),
...
The spatial distribution of cycling potential can be explored interactively by selecting the 'top n' routes with the highest estimated cycing demand.
Information about the *aggregate cycling potential* on on the road network is shown in the Route Network layer.
Because it adds together overlapping from all routes,
this layer results much higher estimates of cycling potential than the route-allocated OD data alone.
Figure 5 which shows output from this layer along Otley Road, where cycling under the Go Dutch scenario rises from 73 to 296 at the route level but 301 to 1133 for the Route Network layer.
The routes were aggregated using the R function `overline` [@lovelace_stplanr:_2016].^[Thanks to Barry Rowlingson from the University of Lancaster for assistance developing this function.]
