...
The inclusion of age specific parameters and the impacts of mode shift from walking the flexibility of the HEAT: modifying the method in this way allows publicly available data to provide detailed evidence for creating local and potentially route-specific business cases. 

The net change in the number of deaths avoided for each OD pair was estimated as the number deaths avoided due to cycle commuting minus the number of additional deaths due to reduced walking.  Note that this approach means that for some OD pairs where walking made up a high proportion of trips, additional deaths were incurred. The monetary value of the mortality impact was calculated by drawing on the standard 'value of a statistical life' used by the Department for Transport.
...
The health benefits of cycling do not rise in proportion to the number of cyclists, or even the number trips cycled: longer trips lead to a greater health benefit than short ones and older people benefit more from increased physical activity. Further, health benefits along desire lines with a low pedestrian mode share can expected to be greater. 

This is illustrated in the data presented by the PCT. The economic value of health benefits reported in Figure 5 (above) is estimated to be £70785, based on the HEAT methodology described in the previous section. Exploration of this variable for other areas shows that, when health savings are the main criteria, longer desire lines would be favoured, as opposed to focussing on the more immediately obvious metric of focussing on the number of additional trips cycled. Future versions of the PCT could allow users to select the 'top n' routes ranked by health and carbon benefits, in addition to the number modelled for who cycling is the main mode.
...
By showing not showing potential health-related benefits the tool provides various metrics for transport planners, going beyond the number of additional trips.
