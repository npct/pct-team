...
The central input dataset for PCT-England was flow data from the 2011 Census, capturing the number of commuters travelling between origin-destination (OD) pairs, disaggregated by mode of travel. We used the open dataset file wu03ew_v2.csv, obtained from the UK Data Service (captured as dataset D1 in Figure 1; see Table 2 for a sample of this dataset)1 This dataset was derived by the Office for National Statistics from responses to questions 40 ("In your main job, what is the address of your workplace?") and 41 ("How do you usually travel to work?") in the English 2011 Census (see ons.gov.uk/census for further information).  We used the version of the dataset in which origins and destinations  were defined using Medium Super Output Areas (MSOAs) (average population: 7,800).  2These Census 2011 OD data were then processed using the R package stplanr (Lovelace et al., 2016), in order to assign information on distance and hilliness.  To each MSOA, we assigned the population-weighted centroids of administrative zones.3  Route distance data were then generated by the route_cyclestreet function in the stplanr R package.  This is based on CycleStreets.net estimate of the ‘fastest’ route.  Route hilliness data was also assigned from CycleStreets.net, as the average gradient (%) of the 'fastest' route.  This generated the dataset D3 in Figure 1; other input datasets included mortality data at the local authority level (dataset D2) and an estimate of the gender composition of cyclists in each flow (dataset D4).
