...
This paper describes the Propensity to Cycle Tool (PCT), an online, interactive evidence-based planning support system to explore possible cycling futures at the local level.
Specifically, the PCT uses origin-destination (OD) data to estimate where cycle traffic has the greatest potential to grow.
A simple but useful model was used to generate these localised 'cycling futures' for commuter trips, as a function of route distance, hilliness and other factors.
More sophisticated and context-specific 'back ends' for the PCT could also be developed.
...
The PCT is completely open source, enabling its rapid deployment other contexts without licensing or cost restrictions.
...
illustrated, for example, by major (yet small in comparison with motorised travel spend) investment
...
Off-road cycle paths can help make cycling a safe, accessible and convenient
option for people of all ages and abilities [@Heinen2015].
Further using modified cycles, such as quadricycles and handcycles, can provide independence and mobility for disabled people [@Aldred2008], where appropriate infrastructure allows.
...
- Estimation and visualisation of cycling potential at 'desire line' level. Unlike area-based estimates, this matches the linear and sometimes lengthy nature of cycling desire lines. The location of lengthy paths cannot easily be prioritised using area-based measures alone.
...
high numbers of 'potentially cycleable trips' in London [@TfL2010].
...
based on current travel behaviour, under various scenarios.
...
decision making.
Therefore, this section should be seen as a case study of one particular approach to modelling particular, context-specific scenarios. Future users of the PCT may choose to use different models of cycling uptake and to examine different scenarios.
