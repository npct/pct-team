...
# downloader::download("https://www.zotero.org/api/groups/418217/collections/TUEXXIV4/items/top?limit=100&format=bibtex&v=1", destfile = "references.bib")
# # 
# # 
...
Modal shift to cycling is an increasingly common objective in transport planning institutions worldwide. 
Evidence suggests that high-quality infrastructure 
can boost local cycling rates, especially when
constructed along
'desire lines' of greatest latent demand.
The importance of building in the right place creates the need for tools to visualise the geographical distribution of cycling potential. 
This paper describes the Propensity to Cycle Tool (PCT), an online, interactive evidence-based planning support system for envisioning the local implications of different regional or national cycling scenarios.
Specifically, the PCT uses origin-destination (OD) data to model where cycle traffic has the greatest potential to grow.
Cycling potential is modelled as a function of route distance, hilliness and other factors at the OD and area level, although more sophisticated models could subsequently be used and visualised throught the PCT.
The most innovative feature of the PCT is its interactive display of the shifting spatial distribution of cycling potential under different scenarios.
Case study cities are used to illustrate the tool's capabilities under four scenarios, including 'Gender Equality' and 'Go Dutch'.
Critically, the accessible, browser-based interface of the PCT
([geo8.webarch.net/master/](http://geo8.webarch.net/master/))
enables public participation in the transport planning process. 
The PCT is completely open source, enabling its rapid deployment new contexts without licensing or cost restrictions.
We conclude that the method can help plan for a future in which
cycling is the natural choice short trips.


```{r, echo=FALSE}
# part of a wider
# shift towards healthy, low carbon transport systems.
```
...
This active travel mode already provides fast, affordable,
and convenient mobility to millions of people each day [@Komanoff2004] and
mounting evidence of the external costs of car-based
...
has pushed cycling further up the
transport agenda in recent years.
Cycling is increasingly central to sustainable transport strategies, as
illustrated, for example, by major investment
strategies worldwide^[Several examples
...
Emerging evidence has demonstrated the important role
that high-quality infrastructure can play in modal shift to cycling [@Parkin_2012].
Off-road cycle paths and interventions a safe, accessible and convenient
transport option for people of all ages and abilities [@Heinen2015].
In addition, modified cycles (e.g. quadricycles and handcycles)
allow cycling to provide an efficient means of self propulsion for people who
...
Thus the question of where to construct new cycle infrastructure
is of strategic importance [@larsen_build_2013] and
highlights the need
for evidence-based tools for active travel [@Payne2014] where
the 'predict and provide' paradigm fails [@na_ess_transport_2014].
Within this context the Propensity to Cycle Tool (PCT)
represents an open source and open access approach to transport modelling,
providing an accessible evidence-base for modals shift.

The PCT can also be framed in the context of planning support systems (PSS).
These decision support tools have recently received attention from transport and
land-use planning communities [@brommelstroet_equip_2010].
Motivated by the observation that transport planning has tended to be
"closed to public over-sight", @golub_making_2013 created a proof-of-concept
tool to enable public engagement with strategic transport plans.
Building on such motivations, the PCT is an interactive map-based PSS
...
[@Pettit2013]. The PCT differs from previous PSS, however, in its:

- Focus on wide ranging scenarios for cycling growth. This expands the 'option space' visible to transport planners, encouraging consideration of short and long-term planning horizons simultaneously.
- Estimation and visualisation of cycling potential at 'desire line' level. Unlike area-based estimates, this matches the linear nature of cycle paths, the location of which cannot be easily prioritised using area-based measures alone.
- Route-allocation of the desire lines. This facilitates the identification of specific segments of the transport network to be targeted for improvement.

The PCT was commissioned by the UK's Department for Transport to identify
"parts of [England] with the greatest propensity to cycle" to help
prioritise strategic investment in active travel [@DepartmentforTransport2015].
Part of the prototype development involved user testing sessions.
Feedback from over 70 practitioners and policy-makers showed that the tool meets
needs not addressed by existing tools
and that it can help decision-making, for example by helping planners decide
what cycling capacity to plan for along a specific route.

The PCT is, to the authors' knowledge, the first online and interactive
planning support system focussed on cycling.
The code underlying the PCT is publicly available at
[github.com/npct](https://github.com/npct) under the open source AGPL licence.
This enables the methods described here to be
reproduced and used by others, while obliging further developments to be shared
with the community. With access to appropriate data (described in the next section)
and programming skills, the PCT can thus be rapidly deployed in new
contexts.
The PCT is built entirely on open source software.
The aggregate level OD model underlying the PCT was written in R
[@RCoreTeam2015] and the
interface was written in **shiny**, an R package for creating online
web applications for data visualisation [@Chang2015].

## The PCT's relation to transport models

The policy relevance of the tool stems from its ability to develop, compare and visualise the local consequences of
various scenarios for cycling futures at city to national levels. 
The PCT is a *strategic* transport planning tool, operating at a higher geographic
level than 'microscopic' transport
models such as SUMO and PTV VISSIM, which simulate vehicular traffic in
real time [@Behrisch2014;@Krajzewicz2014].
The focus on a single mode
enabled rapid and open development of the tool
whilst retaining a relatively simple interface that is focussed on
a single purpose: where to prioritise pro-cycling interventions. 
This makes the PCT scalable to national levels,
while maintaining its ability to estimate cycling potential at
relatively fine-grained (and flexible) levels of geographic resolution.

National-level scenario-based transport models, by contrast,
often lack local geographical
detail [e.g. @McCollum2009].
The PCT approach is thus distinctive in combining strategic overview
with the local (and highly flexible) geographical visualization scenario outcomes.
This makes the tool especially well-suited to local-level analysis of
the impacts of achieving a regional or national target level of cycling
(typically measured as a proportion of all trips).
The 'Government Target' scenario, for example, assumes a doubling in
...
how might this increase be distributed spatially?
...
The evidence-base underlying the PCT's route level scenarios is 'OD data'.
An 'OD pair' in this context can be represented visually as a
desire line: a line connecting the origin
...
Based on this work, the model estimates the rate of cycling
between OD pairs.

The visualisation of cycling desire lines
differs from models of modal shift that operate at
city [@Lovelace2011c] or area levels [@muller2008travel].
In relation to cycling, @Parkin2008 used
...
and the trajectories of cyclists using GPS data, have great potential
for creating an evidence-base for prioritising investment in cycle paths locally
[@Ehrgott2012;@Broach2012].
By allocating modelled demand to the
route network, a method presented by @Zhang2014 was able to
identify routes that would "achieve maximum impacts early on".

A key limitation of such models is that
they do not simulate a range of scenios in the long term.
Indeed, it was found in survey of practitioners that existing tools
"do not sufficiently support the generation of new
strategies" [@brommelstroet_equip_2010].
Moreover, a wider issue with the various transport models
mentioned in this section is that the results they produce
are often inaccessible to transport planners
(e.g. requiring sophisticated software for their interpretation)
...
OD data for each scenario are pre-calculated,
enabling the outcome to be displayed rapidly.
Further, the user is not required to specify any
numerical parameters to interact with
...
A key strength of the PCT is its provision of detailed, interactive results from simple inputs.
The aim is not to produce a full transport-land use model, nor to provide a full mode choice model. Instead, the tool seeks to assist decision makers and planners in considering, visualizing and analyzing what cycling growth might look like – and necessitate in terms of infrastructure provision. Within the English context at least, ambitious local and national cycling targets are often set in terms of mode share, but without any detailed consideration of what this might mean in terms of cycling flows along specific routes. The PCT provides a representation of what, if such targets are achieved, the spatial distribution of cycling trips might look like at the area and network level in practice.  
...
names(flow)[4] <- "Total no. commuters"
names(flow)[5] <- "No. cycle commuters"
...
data("cents")
cents1 <- as.data.frame(cents[1:3,-c(3,4)])
names(cents1) <- c(
  "Area of residence",
  "Area name",
  "Longitude",
...
quality. It worth noting that
a variety of emerging sources could be used instead to
provide OD data. This includes emerging sources of 'Big Data'
[@Kitchin2013] such as: mobile
...
involve traveling on large, busy roads.
...
the basis of a model which estimates the future rate of cycling,
based on current travel behaviour, under various senarios.
...
to be visualised by the PCT. Although the model is important, it is
making-sense of and visualising its output which can lead to improved
decision making. Therefore, this section should be seen as a case study
of model scenarios, rather than the 'official' scenarios used in the PCT.
Different scenarios, and models of cycling uptake, will be appropriate
...
buffer, for example
whener the number of MSOA zones fell below some threshold
...
These can be framed in terms of the removal of different barriers
preventing cycling becoming the natural mode of choice for urban trips:
infrastructural, cultural and technological.
The scenarios are context-specific and based partly on
political commitments to cycling (see *govtarget*).
Use of the PCT in new contexts will benefit from new scenarios that
...
Above-average percentage point increases (i.e. more than a doubling) are projected in areas
...
*godutch* represents what would happen if English people were as likely to cycle a given trip
(in terms of distance and hilliness, in this case) as Dutch people.
...
200 commuters, of which 2 are cyclists ($pcycle_{ab} = 0.01$).
The scenario adds an additional 5% of commuters: an
additional 10 cyclists, i.e. an increase from 2 to 12: more than double.
...
These case study cities were chosen by the Department for Transport to demonstrate the tool's performance in different contexts: one is a polycentric city with a low but growing rate of cycling (Manchester) while the other is monocentric and has a high historical rate of cycling (Norwich).
...
longer (e.g. 'Go Dutch') timescales for local cycling potential along specific routes.
...
- The extension of the tool to take into account additional data.
This could include data on the spatial distribution of air pollution to minimise exposure [@Farrell_Weichenthal_Goldberg_Hatzopoulou_2015];
the location of new developments, such as  houses, schools or shops;
and road traffic speed and volumes.
