...
Yet transport planners lack tools to visualise
the geographical distribution of cycling potential for strategic planning.
This paper describes the Propensity to Cycle Tool (PCT), an
evidence-based support tool for visualising future cycling scenarios.
Specifically, the PCT aims to help  transport authorities operating at
city, region and country levels envision where new cycle trips are likely to
be generated, to prioritise where to prioritise new pro-cycling
infrastructure, investment and policies, for maximum effectiveness.
Case study cities in England are used to illustrate the tool's capabilities, which
uses origin-destination (OD) data on travel patterns as the basis of
of a model which generates future scenarios.
...
sophisticated models (e.g. to estimate the local impact of new cycle paths)
...
[@larsen_build_2013].
In response, professional transport planners and
