...
header-includes: \usepackage{amsmath} \usepackage{longtable} \usepackage{booktabs}
  \usepackage{setspace} \doublespacing \usepackage{lscape}
...
The PCT is an online planning support system funded by the UK's Department for Transport, the primary aim of which is to map where cycling potential is greatest, to optimise infrastracture investments and other cycling interventions (Department for Transport, 2015).
The remit was not to produce a full transport-land use model but to assist decision makers and planners by focussing on cycling.
This involved visualizing, analysing and modelling available data on where people cycle and the routes they take.
The aim of the paper is to illustrate and discuss the potential for online mapping technology to provide a publicly available evidence-base for sustainable transport planning.
...
## Tool and methods for assessing cycling potential

A number of factors influence
the attractiveness of cycling for everyday trips [@Pucher2010].
Factors over which local authorities have some control, as opposed to national policies such 'strict liability' [@fedtke_strict_2003], are primarily related to the transport network.
Reducing speed limits, creating car free zones and improving the urban realm have all be proposed to make places more cyclable. However, the intervention that has received the most attention has been the construction of new cycle paths. With devolved transport budgets, local and regional authorities can often influence the design of strategic cycling networks including the width, quality [@Heath2006;@TfL2014;@WG2014], directness  [@crow2007] and geographic location of the paths.
  Much has written about best practice for the physical design of cycle paths [e.g. @transport_for_london_london_2015] but little work has explicitly tackled the question of where this high quality quality infrastructure should be built [see @Aultman-Hall1997;@Minikel2012].
The PCT focusses explicitly on *where* to build (and intervene in other ways, e.g. by re-allocating space away from motorised traffic and reducing speed limits) rather than *what* to build.

Conventional transport planning tools within the 'predict and provide' paradigm [@na_ess_transport_2014] have limited capacity to inform the decision making process in relation to these issues. There has been a proliferation of research into the impact of infrastructure on
cycling, but little academic work on how to operationalise the knowledge that infrastructure affects uptake for guiding investment by creating tools and frameworks to prioritise investment.
@larsen_build_2013 identified an "absence of research into how 
to  systematically  prioritize  and  locate  facilities  that  are  to  be  built".
There has, however, been research into the related question of cycling potential, which generally fits into the following three categories:


- Area-based measures are based primarily on data at the level of administrative zones. Outputs from these measures can assist with the location of non-linear transport infrastructure such as cycle parking.

- Individual-based measures are based on individual level survey data, typically a household travel survey. These measures can help target specific groups, e.g. through taylored cycled training schemes.

- Route-based measures are the most geographically disagregated level using and producing output at the level of segments of the transport network that can be visualised on a map.
This is the most geographically detailed level of analysis and of most relevance to transport planners designing core cycle networks.

The other line of research linked to the PCT surrounds the idea of Planning Support Systems (PSS). Both areas of literature are briefly reviewed in this section to put the PCT in its wider context.^[Of
course there is some overlap between the analysis of cycling potential at different levels and PSS. @Zhang2014, for example, is primarily focussed on cycling potential but could be used as the basis of a PSS. @Zhang2014 also use data from area, individual and route network levels although the analysis is primarily focussed on on level (the individual).
]
...
Motivated by this observation, an area-based 'prioritization index'
was developed, based on four variables: the area's current level of cycling,
its cycling potential, the number cycle collisions and cyclists' preferences
of the areas that should be upgraded [@larsen_build_2013]. Since then
a number of other methods and tools have been developed to answer the
question, which are outlined below to put the PCT in context
(see Table 1 for a summary).

@Payne2014 uses the concept of 'filtered permeability' to inform the commercially
developed Permeability Assessment Tool (PAT). 
The full methodology underlying the PAT have not, to the author's knowledge, been published and
the only 
@Zhang2014 

### Individiual-level measures of cycling potential

### Route-level measures of cycling potential
...
```{r, echo=FALSE}
df = readr::read_csv("../data-sources/cycle-tools.csv")
# knitr::kable(df)
library(pander)
pander(df, split.table = Inf)
```
...
In the wider field of land-use and transport planning, Planning Support 
Systems (PSS) have been advocated as a way to overcome the limitations of traditional
methods (Brommelstroet, 2010).
PSS typically works by presenting evidence about the characteristics and needs
of areas in an interactive map (Geertman and Stillwell, 2009; Pettit et al., 2013).

In this context the PCT can be understood as a PSS designed to tackle the 'where to build' question.
