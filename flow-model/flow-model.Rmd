...
There is an emerging literature exploring cycling potential.
This links to the question of 'where to build' because 
areas and routes with the highest potential are likely to be cost-effective places for investment from health and emissions perspectives.
...
These these four were aggregated to the level of evenly spread cells covering the study area.
...
```{r, echo=FALSE, fig.cap="The relationship between distance (left) and hilliness (right) and cycling mode share in England based on the the 2011 Census. The plots show actual (blue) vs predicted (red) prevalence of cycling to work among 17,896,135 commuters travelling <30km to work."}
...
and evidence from England overseas
...
This scenario was generated by taking baseline propensity to cycle (see previous section), and applying Dutch scaling factors, generated through analysis of the English and Dutch National Travel Surveys, in which we estimated how much more likely it was that a given trip would be cycled in The Netherlands than in England. We parameterised the scaling factors as a main effect plus an interaction with trip distance, to take into account the fact that the "Dutch multiplier" is greater for shorter trips compared to longer trips.
The scenario level of cycling under Go Dutch is not affected by the current level of cycling.
...
cycles ('ebikes').
This scenario was generated by taking baseline propensity to cycle, applying the Dutch scaling factors described above, and then applying Ebike scaling factors. 
...
Because decisions about where to invest are motivated not just by the potential number of cyclists but also the social impacts we in the first instance modelled health economic and carbon impacts.

An approach based on the the World Health Organization’s Health Economic Assessment Tool ([HEAT](http://www.heatwalkingcycling.org/)) was used to estimate the number of premature deaths avoided due to increased physical activity [@kahlmeier_health_2014]. HEAT estimates of the increase in premature deaths due to a reduction in the number of people walking, in OD pairs where many people walk to work, were also included (see the Appendix).
...
Figure 3 shows a visualisation of the output: 'desire lines' lines with attributes for each OD pair aggregated
...
```{r, echo=FALSE, fig.cap="Illustration of desire lines shown in Figure 3 after they have been allocated to the road network by CycleStreets.net. Purple and turquoise lines represent the 'quiet' and 'fast' routes, respectively."}
...
This means that routes longer than 20 km or with 10 or fewer commuters were omitted.
...
- **Lines**: When lines are displayed on the interactive map, this tab provides the raw data as a table at the OD pair level. Variables shown include the origin and destination zone of the route, current number of commutes by rail, bus, car and bicycle, the increase in cyclists under different scenarios, and geographical data such as straight line distance and hilliness.
...
The number using York Road, highlighted in Figure 8, for example more than triples (from 71 to 236)
...
These results suggest that as cycling grows, policy interventions in Leeds should shift from routes in the Northwest of the city, the area with the highest current level of cycling, to routes that have low current rates of cycling but high potential, such as York Road to the East. Cycle paths built to help achieve ambitious targets, as represented by the Go Dutch scenario, should be of sufficient width to accommodate the estimated flows.
...
The 'quiet' route is longer: 2.6 km
...
resulting in 'circuity', the difference between Euclidean and route distance [see @iacono_access_2008], values of
...
The same research suggests that circuity values
...
Previous work indicates that women and older people have a greater preference for off-road routes and shorter routes [@garrard_promoting_2008;@woodcock_national_2016], suggesting the 'fast route' option could be favourable from an equity perspective.
...
For instance, we envision stakeholders in local government modifying scenarios for their own purposes, and that academics in relevant fields may add new features and develop new use cases of the PCT. Such enhancements could include:
...
These could include the impacts of physical fitness and access to a bicycle.
