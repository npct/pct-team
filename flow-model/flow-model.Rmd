...
The methods and tools for estimating cycling potential outlined in Table 1 were generally created with only a single study region in mind. The benefit of this that they can respond context-specific to practitioner and policy needs.
...
This was loaded from open access file `wu03ew_v2.csv`, provided by the [UK Data Service](http://wicid.ukdataservice.ac.uk/). This captures the number of commuters travelling between Middle Super Output Area zones (MSOAs, average population: 3300), by mode of travel (see Table 2).
 ...
 The latter data was assigned to OD pairs using the "fastest route" option on CycleStreets.net, using the R package stplanr (Lovelace et al., 2016). See the Appendix for further details.
...
The starting point for creating scenario-based 'cycling futures' was to model current cycle commuting in England,
using OD data from the 2011 Census. The proportion of commutes by cycle was estimated as a function of route distance and hilliness using a logistic regression model applied to the individual level travel survey data data.
Parameters estimated by the regression included square and square-root terms for distance, to capture 'distance decay' (Iacono et al 2008). Interaction terms were used to capture the relationship between distance and hilliness.
Model fit is illustrated in Figure 2.  Equations were developed to estimate commuting mode share  among groups not represented in the between-zone OD data: those commuting within one MSOA and those with no fixed workplace.
This model of current cycling levels formed the basis for three of our four subsequent scenarios (Government Target, Go Dutch and Ebikes). Model parameters were derived from English, Dutch and Swiss travel survey data (see the Appendix for details).   
