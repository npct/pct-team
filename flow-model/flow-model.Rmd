...
The central purpose of this paper is highlight the potential of online, interactive and publicly accessible tools to improve the provision of locally specific evidence for transport planning.
...
With the notable exceptions of @larsen_build_2013 and @zhang_prioritizing_2014, this literature provides little in way of *systematic*, *quantitative* or *scalable* evidence for transport planners.
The work can be classified in a number of ways, including by the main method of the tool (e.g. route choice, facility demand or mode choice), the format of its outputs (e.g. spreadsheet results, GIS-based map or on-line, interactive map) or by the main level of the input data used.
...
Again working within academia but also closely focussed on local planning and policy issues,
...
```{r, echo=FALSE, fig.cap="The relationship between distance (left) and hilliness (right) and cycling mode share in England based on the 2011 Census. The plots show actual (blue) vs predicted (red) prevalence of cycling to work among 17,896,135 commuters travelling <30km to work."}
...
cycles ('ebikes').
This scenario was generated by taking baseline propensity to cycle, applying the Dutch scaling factors described above, and then additionally applying Ebike scaling factors. 
...
The spatial distribution of cycling potential differs markedly between scenarios, as illustrated in Figure 7 for the city of Leeds, West Yorkshire. The top 6 OD pairs (a low number was used to focus on the city centre) in Leeds under Government Target are strongly influenced by the current distribution of cycling trips, concentrated in the North of the city (see Figure 3 for comparison with the baseline).
...
Future developments will be facilitated by the open source codebase underlying the PCT (see [github.com/npct](https://github.com/npct)) [@lima_coding_2014].
