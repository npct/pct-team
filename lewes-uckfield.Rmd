...
- Subset cycling desire lines to include only those *whose centrepoint* pass in close proximity to the train line. This was set as 10 km for this report.
...
- Subset the desire lines again, to include only those that are within a threshold angle (set at 30 degrees) their closes segment
...
To find the lines that were close to parallel with the train line, the function `angle_diff()` was developed. All lines within 30 degrees, clockwise or anti-clockwise, to the train line are illustrated in figure \@ref(fig:para).
...
The desire lines that meet the criteria for inclusion in the analysis of parallels are illustrated in the figure below.

```{r, echo=FALSE}
# # angle of route
# start_end = line2points(lewes_uckfield)
# plot(lewes_uckfield)
# plot(start_end, add = T)
# angle_route = line_bearing(lewes_uckfield)
# # get angle of desire lines
# l_lc$angle = line_bearing(l_lc, bidirectional = T)
# l_lc$angle_dif = angle_diff(l = l_lc, angle = angle_route, bidirectional = FALSE)
# l_lc$Parallel_status = "Neither parallel nor proximate"
# l_lc$Parallel_status[l_lc$angle_dif < 30] = "Parallel" 
# 
# tm_shape(l_lc) +
#   tm_lines(col = "Parallel_status")
# plot(lewes_uckfield)
# plot(l_lc[l_lc$Parallel_status == "Parallel",], add = T)


# # l_midpoints = maptools::SpatialLinesMidPoints(l_lc)
