...
```{r segments, fig.cap="Demonstration of the method for splitting the route into segments.", fig.height="2cm"}
...
the green lines were included through the centrepoint selection method but omitted by 'buffer intersection' method.

```{r paranew, fig.cap="Updated 'parallel' desire lines, whose centre-points lie within 5 km of the proposed route (green) and passes within 2 km of the train line (red). The red lines represent the lines selected through this method."}
...
tm_shape(buff_small) + tm_borders(col = "blue", lwd = 3) +
  tm_shape(lewes_uckfield) + tm_lines(lwd = 5, col = "black") +
  tm_shape(l_cp) + tm_dots() +
  tm_shape(l_lc[sel_para & sel_centrepoint5 & !sel_smallbuff,]) + tm_lines(col = "green", lwd = 5) +
